...
{
    int mjr_ver = 0, mnr_ver = 0;
    unsigned char command = CPLD_REVISION_MJR_REG;
    unsigned char command2 = CPLD_REVISION_MNR_REG;
    struct i2c_client *client = to_i2c_client(dev);
    ...
    mnr_ver = i2c_smbus_read_byte_data(client, command2);
    if (mnr_ver < 0) {
        mutex_unlock(&list_lock);
        return mnr_ver;
    }
...
{
    int val = 0, val2 = 0, val3 = 0, b_year = 0, b_month = 0, b_day = 0;
    unsigned char command = CPLD_BUILD_YEAR_REG;
    unsigned char command2 = CPLD_BUILD_MONTH_REG;
    unsigned char command3 = CPLD_BUILD_DAY_REG;
    struct i2c_client *client = to_i2c_client(dev);
    ...
    val3 = i2c_smbus_read_byte_data(client, command3);
    if (val3 < 0) {
        mutex_unlock(&list_lock);
        return val3;
    }
    ...
    b_year = 2000 + (((val & 0xf0) >> 4) * 10) + (val & 0x0f);
    b_month = (((val2 & 0xf0) >> 4) * 10) + (val2 & 0x0f);
    b_day = (((val3 & 0xf0) >> 4) * 10) + (val3 & 0x0f); 
...
static ssize_t fan_ctrl_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (sda->index < PWRGD_VR_P12V_FAN3){
        command = CPLD_FAN_CTL_REG;
        bit = sda->index;
    } else {
        command = CPLD_FAN_STATUS_REG;
...
{
    int val = 0, res = 0, bit = 0, mask = 0;
    unsigned char command = CPLD_TEST_MODE_REG;
    struct i2c_client *client = to_i2c_client(dev);
    struct sensor_device_attribute *sda = to_sensor_dev_attr(attr);
...
static SENSOR_DEVICE_ATTR(psu_budget_mode, S_IRUGO | S_IWUSR, msci_ctl2_read, msci_ctl2_write, PSU_BUDGET_MODE);
...
*/
    ...
    &sensor_dev_attr_psu_budget_mode.dev_attr.attr,
