...
class Component(ComponentBase):
    ...
    CHASSIS_COMPONENTS = [
        ...
        ["CPLD", "CPLD firmware"],
        ["PoE", "Power over Ethernet controller firmware"],
    ]
    ...
    def get_firmware_version(self):
        ...
        if self.index == 3:
            cmdstatus, poe_min = cmd.getstatusoutput('poetool system get_sw_ver | grep swMinorVerNum')
            cmdstatus, poe_maj = cmd.getstatusoutput('poetool system get_sw_ver | grep swMajorNum')
            poe_ver = poe_maj.split(" ")[-1] + "." + poe_min.split(" ")[-1]
            return poe_ver
