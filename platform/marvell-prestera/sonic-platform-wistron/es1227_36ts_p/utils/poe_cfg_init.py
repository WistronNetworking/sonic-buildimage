...
def power():
    ...
    for psu in psu_list:
        if psu.get_presence():
            try:
                ...
                # Decide voltage: [TODO]Default 240W
                voltage_value = PSU240W_VALUE
    ...
    '''
    cmd = "poetool mgmt set_power_banks 1 %s 585 480 0xa" % (
        PSU240W_VALUE)
    log.log_error(cmd)
    ...
    cmd = "poetool mgmt set_power_banks 2 %s 585 480 0xa" % (
        PSU400W_VALUE)
    log.log_error(cmd)
    result, output_str = run_command(cmd)
    if (result == None or int(result) != 0):
...
def main():
    ...
    for key in port_dict:
        ...
        if (int(port_num) <= 23):  # AT port, oper_mode is 9
            oper_mode = "9"
        else:
