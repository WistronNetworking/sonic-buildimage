...
{
    int val = 0;
    int board_id = 0, sku_id = 0;
    unsigned char command = CPLD_HW_REVISION_REG;
    struct i2c_client *client = to_i2c_client(dev);
    const char *board_id_str = "UNKNOWN";
    ...
    board_id = (val & 0x03);         // Bit[1:0] => Board ID
    sku_id   = (val >> 4) & 0x01;   // Bit[4]   => SKU ID
    ...
    return sprintf(buf, "Board ID: %s, SKU ID: %d\n", board_id_str, sku_id);
}
...
{
    int mjr_ver = 0, mnr_ver = 0, dbg_ver = 0;
    unsigned char command = CPLD_MJR_REVISION_REG;
    struct i2c_client *client = to_i2c_client(dev);

    mutex_lock(&list_lock);

    //mjr_ver
    mjr_ver = i2c_smbus_read_byte_data(client, command);
    if (mjr_ver < 0) {
        mutex_unlock(&list_lock);
        return mjr_ver;
    }
    ...
    //mnr_ver
    command = CPLD_MNR_REVISION_REG;
    mnr_ver = i2c_smbus_read_byte_data(client, command);
    if (mnr_ver < 0) {
        mutex_unlock(&list_lock);
        return mnr_ver;
    }
    ...
    //dbg_ver
    command = CPLD_DBG_REVISION_REG;
    dbg_ver = i2c_smbus_read_byte_data(client, command);
    if (dbg_ver < 0) {
        mutex_unlock(&list_lock);
        return dbg_ver;
    }
...
static ssize_t cpld_show_build_date(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    unsigned char command2 = CPLD_BUILD_MON_REG;
    unsigned char command3 = CPLD_BUILD_DATE_REG;
    struct i2c_client *client = to_i2c_client(dev);

    mutex_lock(&list_lock);

    b_year = i2c_smbus_read_byte_data(client, command);
    if (b_year < 0) {
        mutex_unlock(&list_lock);
        return b_year;
    }

    b_month = i2c_smbus_read_byte_data(client, command2);
    if (b_month < 0) {
        mutex_unlock(&list_lock);
        return b_month;
    }

    b_day = i2c_smbus_read_byte_data(client, command3);
    if (b_day < 0) {
        mutex_unlock(&list_lock);
        return b_day;
    }
...
static ssize_t sfp_port_ctrl_read(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct sensor_device_attribute *sda = to_sensor_dev_attr(attr);
    int val = 0, res = 0, bit = (sda->index) % 6, mask = (0x1) << bit;
    unsigned char command = 0;

    if (sda->index / 6 == 0)
        command = CPLD_SFP_PROT1_CTRL_REG;
    else if (sda->index / 6 == 1)
	command = CPLD_SFP_PROT2_CTRL_REG;
    else if (sda->index / 6 == 2)
        command = CPLD_SFP_PROT3_CTRL_REG;
    else if (sda->index / 6 == 3)
        command = CPLD_SFP_PROT4_CTRL_REG;
...
static ssize_t sfp_port_ctrl_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct sensor_device_attribute *sda = to_sensor_dev_attr(attr);
    int val = 0, res = 0, bit = (sda->index) % 6, mask = (0x1) << bit;
...
static SENSOR_DEVICE_ATTR(alm1_led, S_IRUGO | S_IWUSR, sys_led_read, sys_led_write, ALM1_LED);
static SENSOR_DEVICE_ATTR(alm2_led, S_IRUGO | S_IWUSR, sys_led_read, sys_led_write, ALM2_LED);
...
static SENSOR_DEVICE_ATTR(sfp_led_auto, S_IRUGO | S_IWUSR, port_led_auto_read, port_led_auto_write, FRONT_SFP_LED_EN);

static SENSOR_DEVICE_ATTR(poe_en_ctrl, S_IRUGO | S_IWUSR, poe_enable_read, poe_enable_write, 0);

static SENSOR_DEVICE_ATTR(sku1_psu1, S_IRUGO | S_IWUSR, msci_ctl2_read, NULL, SKU1_PSU1);
static SENSOR_DEVICE_ATTR(sku2_psu2, S_IRUGO | S_IWUSR, msci_ctl2_read, NULL, SKU2_PSU2);
...
static SENSOR_DEVICE_ATTR(sfp_p1_tx_disable, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT1_TX_DISABLE);
static SENSOR_DEVICE_ATTR(sfp_p1_rate_sel0, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT1_RATE_SEL0);
static SENSOR_DEVICE_ATTR(sfp_p1_rate_sel1, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT1_RATE_SEL1);
static SENSOR_DEVICE_ATTR(sfp_p1_loss, S_IRUGO, sfp_port_ctrl_read, NULL, PORT1_LOSS);
static SENSOR_DEVICE_ATTR(sfp_p1_present, S_IRUGO, sfp_port_ctrl_read, NULL, PORT1_PRESENT);
static SENSOR_DEVICE_ATTR(sfp_p1_fault, S_IRUGO, sfp_port_ctrl_read, NULL, PORT1_FAULT);
static SENSOR_DEVICE_ATTR(sfp_p2_tx_disable, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT2_TX_DISABLE);
static SENSOR_DEVICE_ATTR(sfp_p2_rate_sel0, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT2_RATE_SEL0);
static SENSOR_DEVICE_ATTR(sfp_p2_rate_sel1, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT2_RATE_SEL1);
static SENSOR_DEVICE_ATTR(sfp_p2_loss, S_IRUGO, sfp_port_ctrl_read, NULL, PORT2_LOSS);
static SENSOR_DEVICE_ATTR(sfp_p2_present, S_IRUGO, sfp_port_ctrl_read, NULL, PORT2_PRESENT);
static SENSOR_DEVICE_ATTR(sfp_p2_fault, S_IRUGO, sfp_port_ctrl_read, NULL, PORT2_FAULT);
static SENSOR_DEVICE_ATTR(sfp_p3_tx_disable, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT3_TX_DISABLE);
static SENSOR_DEVICE_ATTR(sfp_p3_rate_sel0, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT3_RATE_SEL0);
static SENSOR_DEVICE_ATTR(sfp_p3_rate_sel1, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT3_RATE_SEL1);
static SENSOR_DEVICE_ATTR(sfp_p3_loss, S_IRUGO, sfp_port_ctrl_read, NULL, PORT3_LOSS);
static SENSOR_DEVICE_ATTR(sfp_p3_present, S_IRUGO, sfp_port_ctrl_read, NULL, PORT3_PRESENT);
static SENSOR_DEVICE_ATTR(sfp_p3_fault, S_IRUGO, sfp_port_ctrl_read, NULL, PORT3_FAULT);
static SENSOR_DEVICE_ATTR(sfp_p4_tx_disable, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT4_TX_DISABLE);
static SENSOR_DEVICE_ATTR(sfp_p4_rate_sel0, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT4_RATE_SEL0);
static SENSOR_DEVICE_ATTR(sfp_p4_rate_sel1, S_IRUGO | S_IWUSR, sfp_port_ctrl_read, sfp_port_ctrl_write, PORT4_RATE_SEL1);
static SENSOR_DEVICE_ATTR(sfp_p4_loss, S_IRUGO, sfp_port_ctrl_read, NULL, PORT4_LOSS);
static SENSOR_DEVICE_ATTR(sfp_p4_present, S_IRUGO, sfp_port_ctrl_read, NULL, PORT4_PRESENT);
static SENSOR_DEVICE_ATTR(sfp_p4_fault, S_IRUGO, sfp_port_ctrl_read, NULL, PORT4_FAULT);
...
*/
    ...
    &sensor_dev_attr_alm1_led.dev_attr.attr,
    &sensor_dev_attr_sys_led.dev_attr.attr,
    &sensor_dev_attr_alm2_led.dev_attr.attr,

    &sensor_dev_attr_port_led_auto.dev_attr.attr,
    &sensor_dev_attr_sfp_led_auto.dev_attr.attr,

    &sensor_dev_attr_poe_en_ctrl.dev_attr.attr,

    &sensor_dev_attr_sku1_psu1.dev_attr.attr,
    &sensor_dev_attr_sku2_psu2.dev_attr.attr,
    ...
    &sensor_dev_attr_sfp_p1_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp_p1_rate_sel0.dev_attr.attr,
    &sensor_dev_attr_sfp_p1_rate_sel1.dev_attr.attr,
    &sensor_dev_attr_sfp_p1_loss.dev_attr.attr,
    &sensor_dev_attr_sfp_p1_present.dev_attr.attr,
    &sensor_dev_attr_sfp_p1_fault.dev_attr.attr,
    &sensor_dev_attr_sfp_p2_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp_p2_rate_sel0.dev_attr.attr,
    &sensor_dev_attr_sfp_p2_rate_sel1.dev_attr.attr,
    &sensor_dev_attr_sfp_p2_loss.dev_attr.attr,
    &sensor_dev_attr_sfp_p2_present.dev_attr.attr,
    &sensor_dev_attr_sfp_p2_fault.dev_attr.attr,
    &sensor_dev_attr_sfp_p3_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp_p3_rate_sel0.dev_attr.attr,
    &sensor_dev_attr_sfp_p3_rate_sel1.dev_attr.attr,
    &sensor_dev_attr_sfp_p3_loss.dev_attr.attr,
    &sensor_dev_attr_sfp_p3_present.dev_attr.attr,
    &sensor_dev_attr_sfp_p3_fault.dev_attr.attr,
    &sensor_dev_attr_sfp_p4_tx_disable.dev_attr.attr,
    &sensor_dev_attr_sfp_p4_rate_sel0.dev_attr.attr,
    &sensor_dev_attr_sfp_p4_rate_sel1.dev_attr.attr,
    &sensor_dev_attr_sfp_p4_loss.dev_attr.attr,
    &sensor_dev_attr_sfp_p4_present.dev_attr.attr,
    &sensor_dev_attr_sfp_p4_fault.dev_attr.attr,
