...
#define MAX31790_REG_FREQ_CONFIG	0x01
#define MAX31790_REG_FAIL_MASK2		0x12
#define MAX31790_REG_FAIL_MASK1		0x13
#define MAX31790_REG_FAIL_CTRL		0x14
static char *platform;
module_param(platform, charp, 0000);
MODULE_PARM_DESC(platform, "platform name (support ES-1227).");
...
struct max31790_init_data platform_init_data[PLATFORM_LAST - 1] =
{
	{
		.freq = 0xbb,
		.fan_config = {0x68, 0x79, 0x68, 0x68, 0x79, 0x79},
		.fan_dynamics = {0x4c, 0x4c, 0x4c, 0x4c, 0x4c, 0x4c},
#if 0
		.fail_mask = {0x3d, 0x20},
		.fail_ctrl = 0x45,
#endif
...
static int max31790_init_setup(struct i2c_client *client,
				int platform_id)
{
	int i, rv;

	rv = i2c_smbus_write_byte_data(client,
			MAX31790_REG_FREQ_CONFIG,
			platform_init_data[platform_id - 1].freq);
	if (rv < 0)
		return rv;
	...
	for (i = 0; i < NR_CHANNEL; i++) {
		rv = i2c_smbus_write_byte_data(client,
				...
				platform_init_data[platform_id - 1].fan_config[i]);
		if (rv < 0)
			return rv;

		rv = i2c_smbus_write_byte_data(client,
				MAX31790_REG_FAN_DYNAMICS(i),
				platform_init_data[platform_id - 1].fan_dynamics[i]);
		if (rv < 0)
			return rv;
	}
...
#if 0
	rv = i2c_smbus_write_byte_data(client,
			MAX31790_REG_FAIL_MASK2,
			platform_init_data[platform_id - 1].fail_mask[0]);

	rv = i2c_smbus_write_byte_data(client,
			MAX31790_REG_FAIL_MASK1,
			platform_init_data[platform_id - 1].fail_mask[1]);

	rv = i2c_smbus_write_byte_data(client,
			MAX31790_REG_FAIL_CTRL,
			platform_init_data[platform_id - 1].fail_ctrl);
#endif
	return 0;
}
...
static int max31790_probe(struct i2c_client *client)
{
	...
	if (id && id->driver_data != 0) {
		...
		if (!strcmp(platform, "1227"))
			err =  max31790_init_setup(client, PLATFORM_ES1227);
	}
